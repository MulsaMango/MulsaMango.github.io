...
const statuses: ("Pending" | "Critical" | "Success")[] = [
  "Pending",
  "Critical",
  "Success",
];
...
function generateRows(
  ...
  const descriptions = needsRefrigeration
    ? hazardousDescriptions
    : nonHazardousDescriptions;
  return Array.from({ length: count }, (_, i) => {
    const s = seed + i * 7;
    const rawWeight =
      weightMin +
      ((s * 31 + i * 17) %
        Math.max(1, Math.round((weightMax - weightMin) * 100))) /
        100;
    return {
      id: `${prefix}-${i}`,
...
export function generateMockData(
  ...
  const { groupExpanded, contactExpanded, refrigExpanded } = resolveExpandState(
    expandState,
    level,
  );
  ...
  const groups: TransportModeGroup[] = transportConfigs.map((mode, modeIdx) => {
    const contacts: ContactGroup[] = contactConfigs.map(
      (contact, contactIdx) => {
  ...
  return {
    title: "Multi-level grouping simulator prototype",
    totalCount,
    groups,
  };
}
